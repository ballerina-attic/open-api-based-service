...
# OpenAPI
The [OpenAPI specification](https://swagger.io/specification/) (formerly called the Swagger specification) creates RESTful contracts for APIs. It details all resources and operations of APIs in a human and machine-readable format for easy development, discovery, and integration.
The OpenAPI to Ballerina Code Generator can generate Ballerina services from existing OpenAPI definition files.
...
#### Basic structure of the pet store OpenAPI specification
```json
{
  "swagger": "2.0",
  "info": {
...
Ballerina language is capable of understanding OpenAPI specifications. Execute the below command in a Terminal, to generate the web service.
```
...
After creating the Ballerina web service for the given `petstore.json` OpenAPI file, next, the business logic should be implemented for the functionality of each resource. 

To do this, fill the `open-api-based-service/guide/petstore/`ballerina_petstore_impl.bal` file generated by the Ballerina OpenAPI generator with the required details as preferred. 

For simplicity, this guide uses an in-memory map to store the pet data. The following code is the complete implementation of the pet store web service. 
